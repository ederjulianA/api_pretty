...
const createCompleteOrder = async (req, res) => {
  try {
    const { nit_sec, fac_usu_cod_cre, fac_tip_cod, detalles,descuento } = req.body;
    ...
    const result = await orderModel.createCompleteOrder({ nit_sec,fac_usu_cod_cre, fac_tip_cod, detalles, descuento });
    res.status(201).json({
      success: true,
