...
**Puesta en producción (opción A "manda el ERP", Eder 14/sep ~19:45–19:55):** Eder dejó `woocommerce_hold_stock_minutes` en blanco (antes 60; Woo deja de cancelar impagos y de retener 60 min — el vencimiento del ERP ya cancela en Woo) y recreó el cron en hPanel como **Custom** `/opt/alt/php83/usr/bin/php /home/u638783424/domains/prettymakeupcol.com/public_html/wp-cron.php` (`0,30 * * * *`; el anterior, tipo PHP, usaba `/usr/bin/php` = 7.2 y su "View Output" mostraba la página de error crítico de WordPress). Merge `main` `186a270` → `update-app.bat` → `POST /api/pedidos-web/importar-ahora {cursor:'2026-09-09T00:00:00Z', reprocesar:true}`: 23 leídos, 0 errores, 16 s → **REM21 #11266, REM22 #11267, REM23 #11268, REM24 #11271** (vencen 19/sep 19:52; 3 con alerta de saldo: 4672 → −1, 9292 → −10); **#11264 "checkout abandonado"** (5,01 días; si el negocio quiere reservarlo: pasarlo a En espera en wp-admin); segundo pase desde el 3/sep para refrescar #11254/#11259/#11262 → "abandonado". Paridad verificada: 14/14 artículos ERP = Woo.
